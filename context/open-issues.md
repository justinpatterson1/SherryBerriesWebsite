...
- ~~**Dependency scanning:** no CI at all~~ — **RESOLVED 2026-09-12.** [.github/workflows/dependency-audit.yml](../.github/workflows/dependency-audit.yml) runs [scripts/audit-check.mjs](../scripts/audit-check.mjs) on every PR touching the lockfile, on push to master, **and weekly** (the weekly run is the point — a vulnerability disclosed today affects a lockfile nobody is touching). [.github/dependabot.yml](../.github/dependabot.yml) raises the update PRs, grouped so routine patches arrive as one. `npm run audit:check` runs the same gate locally.
  - **The gate is allowlist-based, deliberately.** Four advisories in prisma's tree have no fix short of downgrading to Prisma 6, which would break the schema — so a plain `npm audit` gate would be permanently red, and a check that can never go green is one people learn to ignore. [scripts/audit-allowlist.json](../scripts/audit-allowlist.json) holds the reviewed exceptions, each with a reason and an **expiry date** that fails the build when it lapses, so an accepted risk gets revisited rather than buried.
  - **2026-09-12 state: 18 → 4.** `npm audit fix` cleared 14 (including `nanoid`, `browserslist`, `fast-uri`, `hono`, `@babel/core`, `esbuild`, `vitest`) and moved prisma 7.8.0 → 7.10.0 within its existing range; `package.json` was untouched. The remaining 4 are all the prisma chain (`deepmerge-ts`, `mysql2` ×2, and `@prisma/config`/`prisma` reported via them), all allowlisted: `mysql2` is an unused driver in a Postgres-only app, and `deepmerge-ts` is reached only by the CLI's config loader. **None of the 18 ever executed in a request** — they were build-time and CLI tooling throughout.
  - ⚠ **`npm audit fix` crashes on the local npm** (9.6.6 on Node 22, with extraneous `sharp` wasm packages): `Cannot read properties of null (reading 'edgesOut')`. Use `npx npm@11 audit fix` instead. CI pins Node 22, whose bundled npm is unaffected.
...
### S7. Paid downloads and payment receipts sit in a publicly-served bucket

**Verified 2026-09-16, not assumed:** uploading the aftercare manual and fetching its key straight from `R2_PUBLIC_URL` returned **HTTP 200 with the full 68,743-byte PDF**. A key that does not exist returns 404, so this is real public serving, not a catch-all.

Everything written to `R2_BUCKET_NAME` is reachable by anyone who knows the object key, because that bucket is what `R2_PUBLIC_URL` points at. The only protection is that keys are random UUIDs and never leave the server. That is the long-standing accepted posture for **payment receipts** (documented in [r2.ts](../src/lib/storage/r2.ts)), but it now also covers a **$100 product**: one leaked key — a log line, a database dump, an admin screenshot — and the file is public permanently, with no way to revoke it.

**Fix (mostly owner, code is ready):** [r2.ts](../src/lib/storage/r2.ts) now honours an optional `R2_PRIVATE_BUCKET_NAME`. Create a second R2 bucket with public access **off**, set that variable in `.env` and `.env.production`, and new digital uploads go there instead; reads fall back to the public bucket so files uploaded beforehand keep working. Until it is set, `uploadDigitalAsset` logs a warning on every upload.

- **Still exposed after that:** the one manual already uploaded (re-upload it through the admin form once the private bucket exists), and **all payment receipts**, which still use `R2_BUCKET_NAME`. Moving receipts too would orphan the existing ones, so it needs a migration rather than a flag flip.
...
- ~~All four legal pages publish `sherryvanessanichols@gmail.com`~~ — **PARTLY RESOLVED 2026-09-17** (owner's change): the published address is now `sherryberriesbodyjewelry@gmail.com`, and all four pages read it from a single `LEGAL_CONTACT_EMAIL` in [contact.ts](../src/lib/legal/contact.ts) rather than each declaring its own literal — the same fix already applied to the domain, and for the same reason: four copies drift.
- **Still open:** that published address matches neither `CONTACT_EMAIL` nor the `EMAIL_FROM` sender (`support@shopsherryberries.com`). A customer replying to an order email and a customer following the Returns Policy still reach two different inboxes. The published half is at least consistent with itself now.

**Fix:** set `NEXT_PUBLIC_SITE_URL` in Vercel, and point `CONTACT_EMAIL`/`EMAIL_FROM` at the same mailbox the legal pages publish (or say why they differ).
...
- **2026-09-15** — **The "Berry Baby" category was missing from the homepage grid, and its navbar and footer links were dead.** The category had been renamed in the database — slug `merch` → `berry-baby` — but nothing in `src` followed, so three hardcoded references pointed at a slug that no longer exists.
  - **The homepage tile vanished silently, by design.** `getFeaturedCategories()` matches `FEATURED_CATEGORY_SLUGS` with `slug: { in: [...] }`, and the comment at [home.ts:62](../src/lib/queries/home.ts#L62) says a slug missing from the database "simply leaves the grid one tile shorter" — so the grid rendered 7 of 8 with no error anywhere. That failure mode is why the rename went unnoticed.
  - **The nav and footer links were worse than missing** — `/products?category=merch` is a live link to a category filter that matches nothing, so both surfaces sent customers to an empty listing.
  - **Fixed in three places:** [home.ts:42](../src/lib/queries/home.ts#L42) (`"merch"` → `"berry-baby"`, repositioned to keep the list alphabetical), [footer.tsx:8](../src/components/layout/footer.tsx#L8) and [navbar.tsx:28](../src/components/layout/navbar.tsx#L28). No database change was needed — the database was already correct.
  - **This is the second time these exact links have broken.** Issue 10 (resolved 2026-08-06) fixed the footer to point at `merch` when the old slug went stale. Category slugs are editable through admin CRUD, and **nothing links an admin rename to the hardcoded slugs in `src`** — no foreign key, no test, no build-time check. A renamed category will silently do this again.
  - **⚠ Two further `merch` references were left alone deliberately:**
    - [returns.ts:35](../src/lib/account/returns.ts#L35) — `RETURNABLE_CATEGORY_SLUGS = new Set(["accessories", "merch"])`. Because the allowlist fails closed, **Berry Baby items are currently treated as final sale**, which contradicts the published policy that merchandise stays returnable for 14 days. Changing it alters a **stated returns policy** recorded as the owner's decision of 2026-08-17, so it needs the owner's sign-off, not a drive-by edit. It is moot while Berry Baby has **0 products**, but it will bite the moment one is added.
    - [seed.ts:191](../prisma/seed.ts#L191) — still seeds `slug: "merch"`, so a freshly seeded dev database reproduces this bug exactly. The seed has drifted from production in other ways too (it has no `nipple-rings`, which production has), so bringing one row in line would be a half-fix.
  - **Verification:** `npm test` **211/211**, lint clean (**32 pre-existing a11y warnings, unchanged**), production build **exit 0**. The featured-slug list was queried against the live database and now resolves **8 of 8** tiles, Berry Baby at position 04 with an image set. **Not visually confirmed in a browser.**
...
- **2026-09-16** — *(owner rule change, not an issue fix)* **Change-of-mind returns are gone entirely — returns are now for OUR mistakes only.** Supersedes the "merch and accessories stay returnable" half of the 2026-08-17 entry below. `"Changed Mind"` was removed from `RETURN_REASONS` in [returns.ts](../src/lib/account/returns.ts), so the form no longer offers it on anything and the API no longer accepts it.
  - **Scope:** every category. A tote bag is now treated the same as a belly ring for preference returns — the difference is only *why* we refuse (hygiene vs. simply not taking preference returns), which is what `isFinalSale()` now decides. It no longer gates whether an item can be returned, and its doc comment says so.
  - **⚠ The Sale of Goods Act carve-out is untouched and must stay:** damaged, defective or incorrect items are still always replaced or refunded, postage on us, final sale or not. Removing change-of-mind is lawful; a blanket no-returns term would not be.
  - **The 14-day window changed meaning**, not value: it was a cooling-off period for merch, and is now the window in which a *problem must be reported*. Faults that could not reasonably have been found sooner are still honoured after it. `RETURN_WINDOW_DAYS` stays 14.
  - **Copy updated everywhere it promised otherwise**, because a policy page that still advertised a merch window would be exactly the P1 class of bug this register exists for: the [Returns Policy](../src/lib/legal/returns-policy.ts) (intro, What Can Be Returned, How to Request, Return Shipping, Exchanges; `LAST_UPDATED` → September 16, 2026), the [account returns form](../src/components/account/returns-view.tsx), the [product badge](../src/components/product/product-trust-badges.tsx) ("Easy returns · Unused items within 14 days" → "Returns · Faults or wrong items"), the [cart strip](../src/components/cart/trust-strip.tsx), the [PDP accordion](../src/app/products/[slug]/page.tsx), and the admin [category pill](../src/components/admin/categories-view.tsx).
  - **`allowedReasonsFor(slug)` → `allowedReasons()`** and **`isReasonAllowed(slug, reason)` → `isReasonAllowed(reason)`**: the list no longer varies by category, and a parameter nothing reads is a lie about how the rule works. `CHANGE_OF_MIND_REASON` deleted.
  - **Verification:** two regression tests pin it — the reason list contains no "Changed Mind" for any input, and the published policy must both say change-of-mind is refused *and* no longer contain the old merch-window wording. Typecheck, `npm test` 270/270, lint (33 warnings, unchanged) all clean, and the rendered `/help/returns` page was checked in the browser.
